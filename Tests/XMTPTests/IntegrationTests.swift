...
//		XCTAssertEqual("second", messages[0].body)
	...
	func testStreamingMessagesShouldBeReceived() async throws {
		...
		let alice = try await Client.create(
			account: FakeWallet.generate(),
			options: ClientOptions(api: ClientOptions.Api(env: .local, isSecure: false))
		)
		let bob = try await Client.create(
			account: FakeWallet.generate(),
			options: ClientOptions(api: ClientOptions.Api(env: .local, isSecure: false))
		)
		let transcript = TestTranscript()
		Task(priority: .userInitiated) {
			for try await message in try await alice.conversations.streamAllMessages() {
				if message.senderAddress != alice.address {
					await transcript.add(message.body)
				}
			}
		}
		...
		Task(priority: .userInitiated) {
			for try await message in try await bob.conversations.streamAllMessages() {
				if message.senderAddress != bob.address {
					await transcript.add(message.body)
				}
			}
		}
		...
		let c2 = try await alice.conversations.newConversation(with: bob.address)

		try await delayToPropagate()

		_ = try await c1.send(text: "hello Alice")
		_ = try await c2.send(text: "hello bob")
		_ = try await c1.send(text: "last time")
		...
		XCTAssertEqual("hello bob", messages[1])
		XCTAssertEqual("last time", messages[2])
	}
