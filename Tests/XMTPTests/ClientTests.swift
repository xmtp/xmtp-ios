...
class ClientTests: XCTestCase {
	...
	func testCreatesAClientManually() async throws {
		...
		let inboxId = try await Client.getOrCreateInboxId(api: options.api, address: alix.address)
		let client = try await Client.ffiCreateClient(address: alix.address, clientOptions: options)
		let sigRequest = client.ffiSignatureRequest()
		try await sigRequest!.addEcdsaSignature(signatureBytes: try alix.sign(message: sigRequest!.signatureText()).rawData)
		try await client.ffiRegisterIdentity(signatureRequest: sigRequest!)
		let canMessage = try await client.canMessage(addresses: [client.address])[client.address]
