...
class ClientTests: XCTestCase {
	...
	func testApiClientCacheKeysDifferentConfigurations() async throws {
		...
		XCTAssertNotEqual(key1.stringValue, key2.stringValue, "Cache keys should differ for different environments (local vs dev)")
		XCTAssertNotEqual(key2.stringValue, key3.stringValue, "Cache keys should differ for different environments (dev vs production)")
		XCTAssertNotEqual(key1.stringValue, key3.stringValue, "Cache keys should differ for different environments (local vs production)")
		...
		XCTAssertNotEqual(key8.stringValue, key9.stringValue, "Cache keys should differ when one has appVersion and other doesn't")
		
		// Test 5: Different gatewayHost values (same env, same isSecure, same appVersion)
		let key10 = ApiCacheKey(api: .init(env: .local, isSecure: true, appVersion: nil, gatewayHost: "https://gateway1.example.com"))
		let key11 = ApiCacheKey(api: .init(env: .local, isSecure: true, appVersion: nil, gatewayHost: "https://gateway2.example.com"))
		...
		let key13 = ApiCacheKey(api: .init(env: .local, isSecure: true, appVersion: nil, gatewayHost: "https://gateway.example.com"))
		XCTAssertNotEqual(key12.stringValue, key13.stringValue, "Cache keys should differ when one has gatewayHost and other doesn't")
		
		// Test 7: Multiple parameters different
		let key14 = ApiCacheKey(api: .init(env: .dev, isSecure: true, appVersion: "1.0.0", gatewayHost: "https://gateway.example.com"))
		let key15 = ApiCacheKey(api: .init(env: .local, isSecure: false, appVersion: "2.0.0", gatewayHost: "https://other-gateway.example.com"))
		XCTAssertNotEqual(key14.stringValue, key15.stringValue, "Cache keys should differ when multiple parameters are different")
		
		// Test 8: Completely identical configurations should produce same key
		let key16 = ApiCacheKey(api: .init(env: .local, isSecure: true, appVersion: "1.0.0", gatewayHost: "https://gateway.example.com"))
		let key17 = ApiCacheKey(api: .init(env: .local, isSecure: true, appVersion: "1.0.0", gatewayHost: "https://gateway.example.com"))
		...
		XCTAssertEqual(key18.stringValue, key19.stringValue, "Cache keys should be identical when all optional params are nil")
		
		// Test 10: Edge case - same gatewayHost but different other params should still differ
		let key20 = ApiCacheKey(api: .init(env: .dev, isSecure: true, appVersion: nil, gatewayHost: "https://gateway.example.com"))
		let key21 = ApiCacheKey(api: .init(env: .dev, isSecure: false, appVersion: nil, gatewayHost: "https://gateway.example.com"))
		XCTAssertNotEqual(key20.stringValue, key21.stringValue, "Cache keys should differ even when gatewayHost is same but isSecure differs")
		...
		let key23 = ApiCacheKey(api: .init(env: .production, isSecure: true, appVersion: "3.0.0", gatewayHost: "https://gateway.example.com"))
