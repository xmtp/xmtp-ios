...
class EnrichedMessagesTests: XCTestCase {
	...
	func testLargeMessageSetPerformance() async throws {
		...
		// Measure performance
		let startTime = Date()
		let messages = try await group.findEnrichedMessages()
		...
		let firstPage = try await group.findEnrichedMessages(limit: 10)
		XCTAssertLessThanOrEqual(firstPage.count, 11) // 10 + membership
		
		if let oldestInFirstPage = firstPage.last {
			let secondPage = try await group.findEnrichedMessages(
