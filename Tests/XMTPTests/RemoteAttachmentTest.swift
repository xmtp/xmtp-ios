...
class RemoteAttachmentTests: XCTestCase {
	func testCanUseAttachmentCodec() async throws {
		...
		let content = RemoteAttachment(url: tempFileURL.absoluteString, encryptedEncodedContent: encryptedEncodedContent)
		...
		let encodedContent: EncodedContent = try remoteAttachment.decrypt(payload: Data(contentsOf: tempFileURL))
