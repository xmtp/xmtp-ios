...
public actor Conversations {
	...
	public func groups(
		...
		return try await v3Client.conversations().listGroups(opts: options).map
	...
	public func dms(
		...
		return try await v3Client.conversations().listDms(opts: options).map {
	...
	public func listConversations(
		...
		let ffiConversations = try await v3Client.conversations().list(
			opts: options)
		...
		-> AsyncThrowingStream<DecodedMessage, Error>
	{
		AsyncThrowingStream<DecodedMessage, Error> { continuation in
			@Sendable func forwardStreamToMerged(
				stream: AsyncThrowingStream<DecodedMessage, Error>
			) async {
				do {
	...
	public func streamAllDecryptedMessages(includeGroups: Bool = false)
		-> AsyncThrowingStream<DecryptedMessage, Error>
	{
		AsyncThrowingStream<DecryptedMessage, Error> { continuation in
			@Sendable func forwardStreamToMerged(
				stream: AsyncThrowingStream<DecryptedMessage, Error>
			) async {
				do {
					var iterator = stream.makeAsyncIterator()
					while let element = try await iterator.next() {
	...
	public func streamAll() -> AsyncThrowingStream<Conversation, Error> {
		AsyncThrowingStream<Conversation, Error> { continuation in
			@Sendable func forwardStreamToMerged(
				stream: AsyncThrowingStream<Conversation, Error>
			) async {
				do {
					var iterator = stream.makeAsyncIterator()
					while let element = try await iterator.next() {
